...
@click.group()
def cli():
    """Jimmy — your personal intelligence system."""
    pass
...
def ingest_goodnotes(path):
    ...
    Auto-discovers from iCloud if no path given:
      jimmy ingest goodnotes

    \b
    Or point at any folder of exported PDFs:
      jimmy ingest goodnotes ~/Downloads/GoodNotes
...
def ingest_files(directories, home):
    ...
    Examples:
      jimmy ingest files ~/Documents ~/Desktop
      jimmy ingest files --home
      jimmy ingest files ~/Google\\ Drive
    """
...
def ingest_youtube_liked(json_path, limit):
    ...
    Then: jimmy ingest youtube-liked 'Liked videos.json'
    """
...
# ── BULK IMPORT (NEW SOURCES) ──────────────────────────────────────────────────

@ingest.command(name="youtube-history")
@click.argument("path")
def ingest_youtube_history(path):
    """Ingest YouTube watch/search history from Google Takeout. Pass Takeout/ folder or watch-history.json."""
    from .ingestion.youtube_history import YouTubeHistoryIngester
    console.print(f"[bold blue]Ingesting YouTube history from {path}...[/]")
    try:
        docs = YouTubeHistoryIngester().ingest(path)
        _store_docs(docs, f"YouTube History ({len(docs)} items)")
    except Exception as e:
        console.print(f"[red]{e}[/]")


@ingest.command(name="google-maps")
@click.argument("path")
def ingest_google_maps(path):
    """Ingest Google Maps data from Takeout (saved places, location history)."""
    from .ingestion.google_maps import GoogleMapsIngester
    console.print(f"[bold blue]Ingesting Google Maps from {path}...[/]")
    try:
        docs = GoogleMapsIngester().ingest(path)
        _store_docs(docs, f"Google Maps ({len(docs)} items)")
    except Exception as e:
        console.print(f"[red]{e}[/]")


@ingest.command(name="gmail-takeout")
@click.argument("path")
@click.option("--max-messages", default=2000, help="Max messages to ingest")
def ingest_gmail_takeout(path, max_messages):
    """Ingest Gmail MBOX export from Google Takeout."""
    from .ingestion.gmail_takeout import GmailTakeoutIngester
    console.print(f"[bold blue]Ingesting Gmail MBOX from {path}...[/]")
    try:
        docs = GmailTakeoutIngester().ingest(path, max_messages=max_messages)
        _store_docs(docs, f"Gmail Takeout ({len(docs)} messages)")
    except Exception as e:
        console.print(f"[red]{e}[/]")


@ingest.command(name="claude-export")
@click.argument("path")
def ingest_claude_export(path):
    """Ingest Claude conversation export (JSON)."""
    from .ingestion.claude_export import ClaudeExportIngester
    console.print(f"[bold blue]Ingesting Claude conversations from {path}...[/]")
    try:
        docs = ClaudeExportIngester().ingest(path)
        _store_docs(docs, f"Claude ({len(docs)} conversations)")
    except Exception as e:
        console.print(f"[red]{e}[/]")


@ingest.command(name="chatgpt-export")
@click.argument("path")
def ingest_chatgpt_export(path):
    """Ingest ChatGPT conversation export (conversations.json)."""
    from .ingestion.chatgpt_export import ChatGPTExportIngester
    console.print(f"[bold blue]Ingesting ChatGPT conversations from {path}...[/]")
    try:
        docs = ChatGPTExportIngester().ingest(path)
        _store_docs(docs, f"ChatGPT ({len(docs)} conversations)")
    except Exception as e:
        console.print(f"[red]{e}[/]")


@ingest.command(name="apple-health")
@click.argument("path", default=str(
    __import__("pathlib").Path.home() / "apple_health_export" / "export.xml"
))
def ingest_apple_health(path):
    """Ingest Apple Health XML export. Default: ~/apple_health_export/export.xml"""
    from .ingestion.apple_health import AppleHealthIngester
    console.print(f"[bold blue]Ingesting Apple Health from {path}...[/]")
    try:
        docs = AppleHealthIngester().ingest(path)
        _store_docs(docs, f"Apple Health ({len(docs)} records)")
    except Exception as e:
        console.print(f"[red]{e}[/]")


@ingest.command(name="ocr")
@click.argument("path")
def ingest_ocr(path):
    """OCR images of documents using Claude Vision. Pass a file or folder of images."""
    from .ingestion.document_ocr import DocumentOCRIngester
    console.print(f"[bold blue]Running OCR on {path}...[/]")
    try:
        docs = DocumentOCRIngester().ingest(path)
        _store_docs(docs, f"OCR ({len(docs)} documents)")
    except Exception as e:
        console.print(f"[red]{e}[/]")


@ingest.command(name="github-repos")
@click.argument("repos", nargs=-1)
def ingest_github_repos(repos):
    """Ingest GitHub repos (README, commits, diffs, issues). Pass owner/repo names or omit to auto-discover."""
    from .ingestion.github_repos import GitHubReposIngester
    from .config import GITHUB_TOKEN
    console.print("[bold blue]Ingesting GitHub repos...[/]")
    try:
        ingester = GitHubReposIngester(GITHUB_TOKEN)
        docs = ingester.ingest(list(repos) if repos else None)
        _store_docs(docs, f"GitHub ({len(docs)} items)")
    except Exception as e:
        console.print(f"[red]{e}[/]")


@ingest.command(name="coding-sessions")
@click.argument("path", required=False, default=None)
def ingest_coding_sessions(path):
    """Ingest AI coding session logs (Claude Code, Cursor). Omit path to auto-discover Claude Code sessions."""
    from .ingestion.coding_sessions import CodingSessionsIngester
    console.print("[bold blue]Ingesting coding sessions...[/]")
    try:
        ingester = CodingSessionsIngester()
        if path:
            docs = ingester.ingest(path)
        else:
            docs = ingester.ingest_claude_code()
        _store_docs(docs, f"Coding Sessions ({len(docs)} sessions)")
    except Exception as e:
        console.print(f"[red]{e}[/]")
...
def digest(no_refresh):
    ...
    result = JimmyEngine().digest()
    console.print(Panel(
        Markdown(result["result"]),
        title=f"[bold]🧠 Jimmy Daily Briefing — {datetime.now().strftime('%B %-d, %Y')}[/]",
        border_style="cyan",
    ))
...
def ask(question):
    ...
    from .retrieval.engine import JimmyEngine
    console.print("\n[dim]Searching...[/]\n")
    result = JimmyEngine().ask(question)
...
def context(topic):
    ...
    result = JimmyEngine().context_pack(topic)
    console.print(Panel(
...
def resurface(topic):
    ...
    result = JimmyEngine().resurface(topic)
    console.print(Panel(
        Markdown(result["result"]),
...
def connections(topic):
    ...
    result = JimmyEngine().connections(topic)
    console.print(Panel(
        Markdown(result["result"]),
...
def upcoming(days):
    ...
    result = JimmyEngine().upcoming(days=days)
    console.print(Panel(
        Markdown(result["result"]),
...
def recent(days):
    ...
    result = JimmyEngine().recent(days=days)
    console.print(Panel(
        Markdown(result["result"]),
...
def status():
    ...
    table = Table(title="Jimmy Knowledge Base", show_header=True, header_style="bold")
    ...
    sources = ["canvas", "file", "web", "note", "granola", "youtube", "youtube_liked",
               ...
               "calendar", "gmail", "gdrive", "apple_health", "google_maps", "claude_chat", "chatgpt_chat",
               "coding_session", "whoop"]
    for src in sources:
        try:
...
@cli.command("import-fiji")
@click.argument("path")
def import_fiji(path):
    """Import FIJI alumni Excel file into contacts."""
    from .contacts.importer import import_fiji_excel
    count = import_fiji_excel(path)
    console.print(f"[green]Imported {count} FIJI alumni contacts.[/]")


@cli.command("import-linkedin")
@click.argument("path")
def import_linkedin(path):
    """Import LinkedIn connections CSV into contacts."""
    from .contacts.importer import import_linkedin_csv
    count = import_linkedin_csv(path)
    console.print(f"[green]Imported {count} LinkedIn contacts.[/]")


@cli.command("contacts")
@click.option("--q", default="", help="Search name/company/notes")
@click.option("--aff", default="", help="Filter by affiliation (comma-separated)")
@click.option("--region", default="")
@click.option("--company", default="")
@click.option("--stale", default=0, type=int, help="Days since last contact")
@click.option("--limit", default=50, type=int)
def contacts_cmd(q, aff, region, company, stale, limit):
    """Query your network. Examples:
    jimmy contacts --aff "FIJI,Columbia" --region NYC
    jimmy contacts --stale 180 --aff "family"
    """
    from .contacts.db import query_contacts
    aff_list = [a.strip() for a in aff.split(",") if a.strip()] if aff else None
    results = query_contacts(q=q, affiliations=aff_list, region=region, company=company, stale_days=stale, limit=limit)

    if not results:
        console.print("[dim]No contacts found.[/]")
        return

    table = Table(title=f"Network ({len(results)} results)", show_header=True, header_style="bold")
    table.add_column("Name", style="cyan")
    table.add_column("Company")
    table.add_column("Role")
    table.add_column("Region")
    table.add_column("Closeness", justify="center")
    table.add_column("Tags", style="dim")

    for c in results:
        name = f"{c['first_name']} {c['last_name']}".strip()
        closeness = str(c['closeness']) if c['closeness'] else "-"
        tags = ", ".join(c.get("affiliations", [])[:3])
        table.add_row(name, c['company'], c['role'], c['region'], closeness, tags)
...
def serve(host, port, reload, chroma_port):
    """Start the Jimmy API server (chromadb runs as a separate subprocess)."""
    import os
    import uvicorn
    ...
    console.print(f"[bold green]Jimmy server starting on http://{host}:{port}[/]")
    try:
        uvicorn.run("jimmy.api.server:app", host=host, port=port, reload=reload)
    finally:
        if chroma_proc is not None:
...
def graph(port, chroma_port):
    ...
    console.print(f"[bold green]Starting Jimmy server...[/]")
    ...
    try:
        uvicorn.run("jimmy.api.server:app", host="127.0.0.1", port=port, log_level="error")
    finally:
        if chroma_proc is not None:
            chroma_proc.terminate()
