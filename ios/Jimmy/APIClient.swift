...
enum JimmyError: LocalizedError {
    case badURL
    ...
    var errorDescription: String? {
        ...
        case .serverNotReachable:
            return "Can't reach Jimmy. Make sure the server is running."
...
struct LearnCard: Codable, Identifiable {
    var id: String { (title ?? question ?? statement ?? type) + (emoji ?? "") }
    let type: String          // intro | concept | multiple_choice | true_false | summary
    let title: String?
    let hook: String?
    let body: String?
    ...
    let question: String?
    let options: [String]?
    let answer: AnyCodableAnswer?  // can be String ("B") or Bool (true/false)
    let explanation: String?
    let statement: String?
    let recap: [String]?
    let emoji: String?

    // Convenience for answer type
...
// answer field can be "B" (String) or true (Bool)
enum AnyCodableAnswer: Codable {
    case string(String)
    case bool(Bool)
    ...
    init(from decoder: Decoder) throws {
        ...
        throw DecodingError.typeMismatch(AnyCodableAnswer.self, .init(codingPath: decoder.codingPath, debugDescription: "Expected String or Bool"))
    }
...
class APIClient: ObservableObject {
            ...
            let task = Task {
                do {
                    ...
                    if let http = response as? HTTPURLResponse, http.statusCode != 200 {
                        continuation.finish(throwing: JimmyError.serverError(http.statusCode, "Streaming request failed"))
                        return
                    }
    ...
    func ingestVoiceMemo(_ text: String, durationSeconds: Double = 0) async throws {
        guard let url = URL(string: baseURL + "/ingest/voice") else { throw JimmyError.badURL }
        ...
        guard let http = resp as? HTTPURLResponse, (200...299).contains(http.statusCode) else {
            throw JimmyError.serverError(0, "Voice memo ingest failed")
        }
    }
    ...
    func ingestFile(data: Data, filename: String, mimeType: String) async throws {
        guard let url = URL(string: baseURL + "/ingest/file") else { throw JimmyError.badURL }
        ...
        do {
            ...
            throw JimmyError.serverNotReachable
        }
        if let http = resp as? HTTPURLResponse, http.statusCode != 200 {
            let bodyStr = String(data: respData, encoding: .utf8) ?? ""
            throw JimmyError.serverError(http.statusCode, bodyStr.isEmpty ? "File ingest endpoint not yet available on server" : bodyStr)
        }
    }
    ...
    func learn(topic: String) async throws -> LearnResponse {
        struct Req: Codable { let topic: String }
        return try await post("/learn", body: Req(topic: topic))
    }
    ...
    func ingestVoiceAudioWithResponse(audioData: Data, filename: String, title: String) async throws -> VoiceIngestResponse {
        ...
        if let http = resp as? HTTPURLResponse, http.statusCode != 200 {
            ...
            throw JimmyError.serverError(http.statusCode, bodyStr)
        }
        do {
            return try JSONDecoder().decode(VoiceIngestResponse.self, from: data)
        } catch {
            throw JimmyError.decodingFailed(error.localizedDescription)
        }
    }
    ...
    private func get<T: Decodable>(_ path: String) async throws -> T {
        guard let url = URL(string: baseURL + path) else { throw JimmyError.badURL }
        ...
        if let http = resp as? HTTPURLResponse, http.statusCode != 200 {
            ...
            throw JimmyError.serverError(http.statusCode, body)
        }
        do {
