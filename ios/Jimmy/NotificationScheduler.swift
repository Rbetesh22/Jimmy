...
class NotificationScheduler {
    ...
    func scheduleMorningBriefing(userName: String, hour: Int = 8) {
        let center = UNUserNotificationCenter.current()
        center.removePendingNotificationRequests(withIdentifiers: [JimmyNotification.morningBriefing])
        ...
        let request = UNNotificationRequest(
            identifier: JimmyNotification.morningBriefing,
            content: content,
            trigger: trigger
        )
        center.add(request)
    ...
    func scheduleStudyReminder(examName: String, examDate: Date, daysBefore: Int = 2) {
        ...
        let id = "\(JimmyNotification.studyReminder)_\(examName.replacingOccurrences(of: " ", with: "_"))"
        ...
        content.body  = "Quick review? Open Jimmy to study."
        content.sound = .default
        content.userInfo = [
    ...
    func scheduleStreakReminder(streak: Int) {
        let center = UNUserNotificationCenter.current()
        center.removePendingNotificationRequests(withIdentifiers: [JimmyNotification.streakReminder])
        ...
        let trigger = UNCalendarNotificationTrigger(dateMatching: comps, repeats: true)
        let request = UNNotificationRequest(
            identifier: JimmyNotification.streakReminder,
            content: content,
            trigger: trigger
        )
        center.add(request)
    ...
    func scheduleSRSReminder(dueTopics: [String]) {
        let center = UNUserNotificationCenter.current()
        center.removePendingNotificationRequests(withIdentifiers: [JimmyNotification.srsReminder])
        ...
        let trigger = UNCalendarNotificationTrigger(dateMatching: comps, repeats: true)
        let request = UNNotificationRequest(
            identifier: JimmyNotification.srsReminder,
            content: content,
            trigger: trigger
        )
        center.add(request)
    ...
    func scheduleRandomResurface(snippet: String) {
        let center = UNUserNotificationCenter.current()
        center.removePendingNotificationRequests(withIdentifiers: [JimmyNotification.randomResurface])
        ...
        let trigger = UNCalendarNotificationTrigger(dateMatching: comps, repeats: true)
        let request = UNNotificationRequest(
            identifier: JimmyNotification.randomResurface,
            content: content,
            trigger: trigger
        )
        center.add(request)
    ...
    func sendTestNotification() {
        ...
        content.body  = "Jimmy notifications are working correctly."
        content.sound = .default
    ...
    func cancelMorningBriefing() {
        UNUserNotificationCenter.current().removePendingNotificationRequests(
            withIdentifiers: [JimmyNotification.morningBriefing]
        )
    }

    func cancelStreakReminder() {
        UNUserNotificationCenter.current().removePendingNotificationRequests(
            withIdentifiers: [JimmyNotification.streakReminder]
        )
    }

    func cancelSRSReminder() {
        UNUserNotificationCenter.current().removePendingNotificationRequests(
            withIdentifiers: [JimmyNotification.srsReminder]
        )
    }

    func cancelRandomResurface() {
        UNUserNotificationCenter.current().removePendingNotificationRequests(
            withIdentifiers: [JimmyNotification.randomResurface]
        )
    }
