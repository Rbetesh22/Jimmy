...
struct HomeView: View {
                ...
                LazyVStack(alignment: .leading, spacing: 0) {
                    ...
                    if isLoading {
                        ...
                        if !recommendations.isEmpty {
                            ...
                            .padding(.bottom, 16)
                            .opacity(contentAppeared ? 1 : 0)
                            .animation(.easeInOut(duration: 0.3).delay(0.36), value: contentAppeared)
                        }
    ...
    private func loadAll() async {
        ...
        // Recommendations load lazily
        Task {
            if let r = try? await api.recommendations() { recommendations = r.recommendations }
        }
    }
...
struct LifeCountdownsCard: View {
    ...
    private var datadogDays: Int {
        let target = Calendar.current.date(from: DateComponents(year: 2026, month: 7, day: 6))!
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 32) {
                ...
                VStack(spacing: 12) {
                    ShareLink(item: "I'm on a \(streak)-day learning streak with Jimmy — my AI second brain. 🔥") {
                        HStack(spacing: 8) {
