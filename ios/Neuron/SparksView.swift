...
struct SparkCard: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if expanded {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    // "Ask Neuron about this" — full width accent button
                    Button {
                    ...
                    HStack(spacing: 8) {
                        ...
                        if let title = spark.title, let conn = spark.connection {
                            ShareLink(
                                item: "\(title)\n\n\(conn)\n\n— Neuron, my second brain",
                                subject: Text("Connection from Neuron"),
                                message: Text(title)
                            ) {
                                Image(systemName: "square.and.arrow.up")
                                    .font(.system(size: 14))
    ...
    private func shareSparkCard() {
        let text = "Connection: \(spark.title ?? "")\n\n\(spark.connection ?? "")\n\nDiscovered by Neuron — your second brain"
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            VStack(spacing: 5) {
                ...
                Text("Check that your Neuron server is running")
                    .font(.system(size: 15))
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
                    .opacity(appeared ? 1 : 0)
...
struct EmptySparksView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            VStack(spacing: 5) {
                Text("New connections appear as you use Neuron")
                    .font(.system(size: 17, weight: .semibold))
                    .multilineTextAlignment(.center)
                    .opacity(appeared ? 1 : 0)
                    .animation(.easeInOut(duration: 0.3).delay(0.15), value: appeared)
