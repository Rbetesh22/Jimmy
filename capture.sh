#!/bin/bash
# ~/neuron/capture.sh
# Lightweight capture to Neuron from the command line or a macOS Quick Action.
#
# Usage:
...
#
# Set NEURON_HOST to override the server address (default: http://localhost:7700).

NEURON_HOST="${NEURON_HOST:-http://localhost:7700}"

notify() {
    local msg="$1"
    osascript -e "display notification \"$msg\" with title \"Neuron\"" 2>/dev/null || true
}

# ── Check server is up ─────────────────────────────────────────────────────────
if ! curl -s --max-time 3 "${NEURON_HOST}/health" > /dev/null 2>&1; then
    echo "Error: Neuron server is not running at ${NEURON_HOST}" >&2
    echo "Start it with: cd ~/neuron && neuron serve" >&2
    notify "Neuron server not running"
    exit 1
fi
...
if [[ "$1" == "--text" ]]; then
    ...
    RESULT=$(curl -s -X POST "${NEURON_HOST}/ingest/text" \
    ...
    if echo "$RESULT" | grep -q '"ok": *true'; then
        ...
        notify "Text captured to Neuron ($CHUNKS chunks)"
    ...
    RESULT=$(curl -s --max-time 60 -X POST "${NEURON_HOST}/ingest/youtube" \
        -H "Content-Type: application/json" \
        -d "{\"url\": \"$URL\"}")
    if echo "$RESULT" | grep -q '"ok": *true'; then
        CHUNKS=$(echo "$RESULT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('chunks', '?'))" 2>/dev/null || echo "?")
    ...
    echo "Ingesting: $URL"
    RESULT=$(curl -s --max-time 30 -X POST "${NEURON_HOST}/ingest/url" \
        -H "Content-Type: application/json" \
        -d "{\"url\": \"$URL\"}")
    if echo "$RESULT" | grep -q '"ok": *true'; then
        CHUNKS=$(echo "$RESULT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('chunks', '?'))" 2>/dev/null || echo "?")
        ...
        TAG_DISPLAY="${TAG:+ [$TAG]}"
        notify "Captured to Neuron: $TITLE ($CHUNKS chunks)$TAG_DISPLAY"
